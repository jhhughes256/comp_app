# Reallocation function which outputs the change in value not the composition
# -----------------------------------------------------------------------------
# This script describes a function similar to that seen in:
#   05_reallocation_function.R
# The main difference is that it outputs the change in composition rather than
#   the final composition. Tests are the same, output now must be added to 
#   the original composition to give the final composition.
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
# Prepare workspace
...
# See if function gives same value as m1 (reallocation according to original)
  fn.m1 <- m.vec + reallocation(m.vec, r.vec1, 1440, hm1)
...
#   SLP SED LPA MVPA
  ...
  fn.m2 <- m.vec + reallocation(m.vec, r.vec2, 1440, hm2)
...
#   SLP SED LPA MVPA
  ...
  fn.m3 <- m.vec + reallocation(m.vec, r.vec3, 1440, hm3)
