...
# Uses models to predict the outcome for a variety of time-use compositions.
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
# Predicted number of shuttles for mean situation
  m.fitness <- predict(model, 
    newdata = list(
      ilr.comp = ilr(m), sex = "1", age = mean(age), ses = mean(ses)
    )
  )

# To capture the scenario of the "pivot coordinate", one compositional part is 
# increased, whilst decreasing the remaining parts by the same factor.
# This is called a reallocated composition.
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
# "One-for-Remaining" Reallocation
